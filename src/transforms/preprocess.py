...
def physical_resample(
    ...
    img_resampled = cv2.resize(img, (new_w, new_h), interpolation=cv2.INTER_LINEAR)
    mask_resampled = None
    if mask is not None:
        mask_resampled = cv2.resize(mask, (new_w, new_h), interpolation=cv2.INTER_NEAREST)
    return img_resampled, mask_resampled
...
class BasePreprocess:
    ...
    def __call__(self, image: np.ndarray, mask: Optional[np.ndarray], label: int, pid: str, spacing: float = None) -> Dict[str, Any]:
