...
from .preprocess import (
    percentile_clip, scale_to_01, physical_resample, resize_image_and_mask, DatasetZScore, apply_clahe
)
...
class TrainTransform:
    def __init__(
        ...
        if level == "strong" and strong_params is not None:
            self.params.update(strong_params)

        self.znorm = DatasetZScore(zscore_stats["mean"], zscore_stats["std"]) if zscore_stats else None

    def __call__(self, image: np.ndarray, mask: Optional[np.ndarray], label: int, pid: str,
                 spacing: Optional[float] = None) -> Dict[str, Any]:
        image = percentile_clip(image, self.clip_percentiles[0], self.clip_percentiles[1])
        image = scale_to_01(image)

        if spacing is not None:
            image, mask = physical_resample(image, mask, spacing, self.target_spacing)
        ...
        image = random_motion_blur(image, prob=self.params["motion_blur_prob"])
...
def build_transforms(cfg, split: str, zscore_stats: Optional[dict] = None):
    ...
    target_spacing = float(cfg["PREPROCESS"].get("TARGET_SPACING", 0.12))
